    ...
    async (client: Client, button: ButtonInteraction<"cached">) => {
        ...
        const response = await button.reply({
            ...
            embeds: [
                client.storage.embeds.default(
                    member,
                    "Управление",
                    `**укажите** пользователя, которого вы хотите **замьютить** / **размьютить** в ${voice}`,
                ),
            ],
            components: [
        ...
        collector.on(
            ...
            async (i: UserSelectMenuInteraction<"cached">) => {
                    ...
                    return i.reply({
                        embeds: [
                            client.storage.embeds.default(
                                member,
                                "Управление",
                                `вы **не** можете **замьютить** / **размьютить** себя!`,
                            ),
                        ],
                        flags: MessageFlags.Ephemeral,
                    });
                ...
                let canSpeak = voice.permissionOverwrites.cache
                    .get(choosed.id)
                    ?.allow.has("Speak");
                let newPermission = !canSpeak;
                ...
                await i.update({
                    ...
                    embeds: [
                        client.storage.embeds.default(
                            member,
                            "Управление",
                            `Теперь ${choosed.user} ${newPermission ? "" : "не "}сможет говорить в ${voice}!`,
                        ),
                    ],
                });
