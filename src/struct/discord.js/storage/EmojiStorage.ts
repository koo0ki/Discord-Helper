...
export class EmojiStorage extends EventEmitter {
    guilds: Collection<string, Guild>;
    cache = new Collection<string, GuildEmoji>();
    md5Map = new Collection<string, string>();
    readonly dir: string = `./assets/emojis/`;
    client: Client;

    constructor(
        client: Client,
        public options = { guildscount: 1 },
    ) {
        super();
        this.client = client;
    ...
    async checkGuilds() {
        ...
        for (let i = 0; i < guilds.length; i++) {
            ...
            let invite =
                (
                    await (
                        guild.channels.cache.last() as TextChannel
                    ).fetchInvites()
                ).first() ||
                (await (
            ...
            this.client.logger.info(`Invite for ${guild.name}: ${invite.url}`);
    ...
    async createGuild(number: number) {
        ...
        this.client.logger.info(`Creating server ${name}...`);
        let guild = await this.client.guilds.create({
            name,
        });
        this.guilds.set(guild.id, guild);
        this.client.logger.info(`Server ${name} created`);
    }
    ...
    async checkEmojisLoop() {
        ...
        for (let i = 0; i < md5RematchQuerycopy.length; i++) {
            ...
            let last = this.md5Map.findKey(
                (i) => i === (item as string).split(".")[0],
            ) as string;
            try {
    ...
    async init() {
        ...
        this.client.logger.info(`Loading emojis...`);
        await this.checkEmojis();
        this.client.logger.info(`Emojis loaded`);
        this.checkEmojisLoop();
    ...
    async checkEmojisInServer(files: string[]) {
        let emojis = this.cache
            .filter(
                (f) =>
                    !files.find((file) => {
                        let hash = this.md5Map.findKey(
                            (item) => item === file.split(".")[0],
                        ) as string;
    ...
    async createEmoji(file: string) {
        ...
        if (!guild)
            return this.client.logger.info(
                `Could not find guild with emoji slot`,
            );
        ...
        for (let i = 0; i < guilds.length; i++) {
            let emoji = guilds[i].emojis.cache.find(
                (emoji) => emoji.name === md5content,
            );
            if (emoji) {
        ...
        this.cache.set(file.split(".")[0], emoji);
        this.client.logger.info(`Emoji ${emoji.name} created in ${guild.name}`);
    ...
    async deleteEmoji(emoji: GuildEmoji, cache: boolean = false) {
        this.client.logger.info(
            `Emoji ${emoji.name} deleted in ${emoji.guild.name}`,
        );
        if (emoji.deletable) {
